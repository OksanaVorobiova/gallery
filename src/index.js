...
// returns popular images after page download
//if no search query was entered in a search field
async function getPopularImg() {
  const response = await imagesAPI.getPopularImages();

  imagesAPI.incrementPage();
  loadMoreBtn.style.display = 'inline-block';
  return isResponseEmpty(response.data.hits);
}
...
function isResponseEmpty(data) {
  if (data.length !== 0) {
    galleryEl.insertAdjacentHTML(
      'beforeend',
      rendering.reduceImagesArrayToMarkup(data)
    );
...
// scrolls document to new images that are already rendered
